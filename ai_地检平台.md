...
当前Can卡的Demo 演示虚拟设备，我已经更新了pycan库，更新后can的demo厂商，支持跨进程通讯，相当于虚拟can。
现在打开虚拟can，会持续发送遥测消息，去掉这个功能，把虚拟can（demo，演示）和其他的can设备一样处理就行。

→ 已改：去掉采集进程对 vendor=0 每秒注入样例遥测帧的逻辑。演示/虚拟 CAN 与其它厂商一样只 ``recv`` 真实帧。
