...
components/Payload/             # 地检共用组件
├── SerialConnectDialog.vue         # 串口连接（free/preset）
├── CanConnectDialog.vue            # CAN 连接
├── UdpConnectDialog.vue            # UDP 连接
├── PayloadTransferInfo.vue         # 传输信息（按 source/设备 IO）
├── IoLogPanel.vue / RawDataSendPanel.vue / CameraImageView.vue
utils/
├── pipelineTips.js                 # 组装器/解释器统一 tooltip 文案
├── deviceSnapshotCache.js          # 设备快照 / 活动设备缓存
└── telemetryCfgCache.js            # 遥测配置本地缓存
views/dashboard/index.vue       # 首页 · 设备服务（已开连接列表）
views/payload/
├── telecontrol/
│   ├── control/index.vue           # 控制开关（BIU/XL 共用）
│   ├── command/index.vue           # 遥控（树形 + 指令卡片 + 历史）
│   └── sequence/index.vue          # 指令序列
├── telemetry/
│   ├── table/index.vue             # 实时数据
│   ├── curve/index.vue             # 实时曲线
│   ├── canHistory/index.vue        # 历史 CAN 表回放
│   ├── archive/index.vue           # 历史 CAN 曲线
│   ├── fileHistory/index.vue       # 历史文件表回放
│   └── fileCurve/index.vue         # 历史文件曲线
├── board/
│   ├── camera/index.vue            # 相机 v1.6（与 v1.7 共用实现）
│   ├── camera_v17/index.vue        # 相机 v1.7 薄壳
│   ├── XlBoardPage.vue             # 热控电机 / CPA-ZK / 地检板
│   ├── rkdj/index.vue / zk/index.vue / dj/index.vue
├── debug/
│   ├── simulate/index.vue          # 数据模拟
│   ├── xfer/index.vue              # 数据收发
│   ├── config/index.vue            # 配置文件在线编辑
│   └── tmCalc/index.vue            # 遥测计算
├── lvds/
...
| D7   | 会话同时绑定 `assemblerId`（默认透传）与 `parserId`；收包先组装再解析。                 |
| D8   | 串口 IO 双写：`payload:serial:{port}:io` +（非 home 来源时）`payload:source:{source}:io`。 |
| D9   | 首页设备服务无卡片边框；新建 CAN/UDP/串口弹窗独立组件，与单板复用串口对话框。           |
