...
- **相机图像协议**（见 [07 章](./07-遥控帧组装与遥测解析规则.md)）：
  - 帧头 `EB 90`，类型 `D6`；请求 10 字节，应答 266 字节（256 字节数据区 + 校验）；
  - 帧标识按位（首/中/尾）；v1.6 固定分片拼图，v1.7 按应答有效长度字段拼图；
  - 分辨率 v1.6：400/256/128/64；v1.7：边长 8–400（8 的倍数）；
  - 单帧失败重试 5 次，超限整图重采。
- **插件映射**：`camera_image` 与 `camera_image_v17` 两个 `source` 均注册到同一采集插件 `camera_image`；组装器分别为 `camera_image_d6` / `camera_image_d6_v17`。
- **写 Redis**：将整帧图像写入 Redis；前端拉取展示。
- **全双工**：`cfg_device_connect.json` 中 `fullDuplex: true` 的串口/CAN/UDP 在采集进程内**独立收流线程**，发送不阻塞接收（`base_collector._rx_thread`）。
- **IO 日志**：原始字节先入进程内**内存环缓**（`deque`），按需批量刷入 Redis `payload:{id}:io`；单板「传输信息」Recv 行展示**解释器拆出后的完整帧**（`assembled=true` 时不重复打印 CAN ID）。
...
| 网络     | `udp:{ip}:{port}` 或 `tcp:{ip}:{port}` | `udp:127.0.0.1:66`（**不再**使用 `net:` 前缀） |
...
| 原始收发 IO  | `payload:{id}:io`                            | List(JSON)  | 采集进程 | 主进程   | 调试/传输信息；串口可双写见下         |
| 按来源 IO    | `payload:source:{source}:io`                 | List(JSON)  | 采集进程 | 主进程   | 单板按功能来源聚合（非 home 时双写）  |

> **现行键与 TTL 以 [13-Redis功能与键一览](./13-Redis功能与键一览.md) 为准**（含会话 `assemblerId`/`parserId`/`source`、遥测按 `dataSub` 共享热键等）。上表部分历史键名（如按设备分的 `tm`）已迁移，请交叉核对。
...
> 数值优先取解析结果 `calc_val`（公式后、值映射前），并按 `CURVE_MAX_POINTS` 裁剪；与前端是否显示曲线无关。
