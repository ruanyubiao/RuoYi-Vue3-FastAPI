...
3. **就地解析**：取帧内「数据类型字节」（见 [07 章](./07-遥控帧组装与遥测解析规则.md)），调用 `telemetryparser`
   解析为字段行，写入 Redis 实时值与（订阅的）曲线时间序列；
...
| 设备类型 | 唯一标识格式                          | 示例                  |
| -------- | ------------------------------------- | --------------------- |
| CAN 卡   | `can:{vendor}:{dev_index}`            | `can:2:0`             |
| CAN 通道 | `can:{vendor}:{dev_index}:{can_index}`| `can:2:0:1`           |
...
**遥测最新值** `payload:can:2:0:1:tm:FF`（JSON）：
...
**指令下发** `LPUSH payload:can:2:0:1:cmd`（JSON）：
...
**指令执行结果** `payload:can:2:0:1:cmd:result:uuid-xxxx`（JSON）：
...
> **曲线时间序列**：采集进程仅对「被订阅」的遥测量写入 ZSet/Stream（score=时间戳），
> 并按容量上限裁剪；前端确认订阅后，主进程把订阅信息写入 Redis 供采集进程读取。
> 详见 [05 章 遥测曲线](./05-前端页面设计.md) 与 [04 章接口](./04-后端接口设计.md)。
