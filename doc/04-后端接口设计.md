...
| `telemetry_controller`     | `/payload/telemetry`    | 遥测表/遥测量查询、曲线数据、订阅          |
...
> 行为：管理器按「卡」聚合——`can:2:0` 进程若已存在则追加通道 `canIndex=1`；否则新建进程。
> 关闭时若该卡所有通道关闭则回收进程（见 [02 章进程管理](./02-数据采集层设计.md)）。

### 设备状态

`GET /payload/device/status?deviceId=can:2:0:1`
...
| POST | `/payload/telemetry/curve/subscribe`   | 订阅某遥测量（开始记录时间序列）             | `payload:telemetry:curve` |
| GET  | `/payload/telemetry/curve/data`        | 拉取某遥测量的曲线数据点                     | `payload:telemetry:curve` |

### 取遥测表最新值

`GET /payload/telemetry/table?deviceId=can:2:0:1&type=FF`
...
sequenceDiagram
  ...
  FE->>API: POST curve/subscribe {type:FF, field:JGB027}
  API->>R: 记录订阅(采集进程据此写时间序列)
  loop 定时
    Col->>R: ZADD payload:{id}:curve:FF:JGB027 score=ts value
    FE->>API: GET curve/data?type=FF&field=JGB027&from&to
    API->>R: ZRANGEBYSCORE
    API-->>FE: [{t, v}, ...]
  end
```
