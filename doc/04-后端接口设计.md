...
| POST | `/payload/telemetry/dev/inject`（或 can-yc） | 开发：注入 CAN 遥测复合帧 HEX           | 开发权限                   |
| POST | `/payload/telemetry/dev/pipeline`      | 开发：HEX → 组装器 → 解释器 → 写 Redis     | 开发权限                   |
| POST | `/payload/telemetry/calc`              | 调试：单字段 Hex → `parse_line_hex`，写历史 | `payload:tmcalc:view`     |
...
> 开发注入路径以实现为准（`payload_telemetry_controller` 中 `/dev/*`）。通用管道用于「数据模拟」页。曲线数值优先使用解析字段 `calc_val`（公式后、值映射前）。
