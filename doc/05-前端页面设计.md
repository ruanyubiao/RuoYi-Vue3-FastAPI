...
- 顶部控件：**遥测表下拉**（数据类型）→ **遥测量下拉**（随遥测表切换）→ **确认** 按钮。
- 点击「确认」后：按间隔调用 `telemetry/curve/data` 拉取该遥测量数据点，绘制曲线（折线）。
- **缩放**：ECharts `dataZoom`，支持选中一段区间放大/缩小。
- **入口二**：从遥测表页点数值进入时，下拉自动选中对应表/量，且视为已点确认、立即开始绘制。
- 订阅：进入时调 `telemetry/curve/subscribe`，通知采集进程开始记录该遥测量时间序列。
