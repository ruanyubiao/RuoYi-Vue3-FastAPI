...
| `number`     | 若 `formula` 非空则先对 UI 值求值（`D` 占位），再按 `dataType` 大端编码 |
...
实现：`module_payload/cfg/hex_text.py` ↔ 前端 `payloadRawData.js`。

| 规则 | 说明 |
| ---- | ---- |
| 分隔 | 空白（空格/Tab/换行）为 token 分隔符，**不是**可忽略字符 |
| token | 每个 token 独立成对；奇数位末字节前补 `0`（`C` → `0C`） |
| 禁止 | 不允许 `0x` 前缀（`0xEB` 非法） |
| 示例 | `A B` → `0A 0B`（不是 `AB`）；`aabbc` → `AA BB 0C` |

遥控页参数预览、开发模拟、IO 日志展示均遵循同一套格式化。

### 1.5 广播帧（广播帧识别 / 指令序列排除）
...
> 源：`test/showimg/serial_image_viewer.py`、`module_payload/assemblers/camera_image_d6_common.py`。  
> 串口参数：波特率 2_000_000（可选 11_000_000）、8 数据位、奇校验(ODD)、1 停止位。  
> v1.6 / v1.7 **共用**请求帧格式与帧标识按位规则；应答解析与分辨率推断见 §3.3。

### 3.1 帧格式

| 帧       | 长度    | 结构                                                                 |
| -------- | ------- | -------------------------------------------------------------------- |
| 请求帧   | 10 字节 | `EB 90` + 类型`D6` + 帧标识 + 数据长(2B) + 序号(2B) + 图像号(1B) + 校验 |
| 应答帧   | 266 字节| `EB 90` + 类型`D6` + 帧标识 + … + 序号(2B) + 图像号 + **256B 数据区** + 校验 |
...
| ---- | ------ | -------------------------------- |
| bit2 | 首帧   | 仅首帧置 1 → `0x04`              |
| bit1 | 中间帧 | 仅中间帧置 1 → `0x02`            |
| bit0 | 尾帧   | 仅尾帧置 1 → `0x01`；单包时与首帧同时为 1 → `0x05` |

- **归类顺序**：首帧 → 尾帧 → 中间帧（`classify_frame_id`）；至少一位有效才算合法。
- **校验**：从类型字节到数据区末尾的字节累加和低 8 位（`sum & 0xFF`）。

### 3.2 拉图请求规划（`plan_d6_image_requests`）

总像素 `N = W×H`，每包有效数据最多 256 字节：

| 情况 | 请求序列 |
| ---- | -------- |
| `N ≤ 256`（如 8×8、16×16） | **单包**：帧标识同时置首+尾位，seq=0 |
| `N > 256` 且整除 256 | 首帧 seq=0 → 中间帧 seq=1..k-2 → 尾帧 seq=k-1 |
| 有余数 | 块数向上取整（如 80×80=6400 → 25 包） |

实现：`camera_image_d6`（v1.6）、`camera_image_d6_v17`（v1.7）组装器；采集插件 `camera_image`（两版 `source` 均映射到同一插件）。

### 3.3 v1.6 与 v1.7 差异

| 项 | v1.6 | v1.7 |
| -- | ---- | ---- |
| 遥控/遥测配置 | `XL-Camera-TeleControlCfg.json` / `XL-Camera-TeleMetryCfg.json` | `XL-Camera-V17-TeleControlCfg.json` / `XL-Camera-V17-TeleMetryCfg.json` |
| 遥测表键 | `D8` / `D9` | `D8V17` / `D9V17` |
| 解释器 | `tm_xl_camera` | `tm_xl_camera_v17`（公共基类 `CameraTmIngestBase`） |
| 图像组装器 | `camera_image_d6` | `camera_image_d6_v17` |
| 应答有效长度 | 固定按 256B 分片拼图 | 字节 4–5 为长度编码 **L**，有效字节数 = **L+1**（1..256）；不足部分 0 填充 |
| 分辨率 UI | 下拉：400/256/128/64 | 输入框：8–400、步进 8（边长像素） |
| 分辨率来源 | 遥测 CAM036/CAM038（D8）或 CAMF029/CAMF027（D9） | 同上字段，解析为 `W×H` 像素边长 |
...
- 总像素 = W×H；v1.6 常用 400/256/128/64；v1.7 支持任意正方形边长（8 的倍数，≤400）。
- 图像号 1~64；单帧失败重试 5 次，超限整图重采。
- 重组后为 `uint8` 灰度图（`reshape(H, W)`），转 PNG 供前端展示；Redis 键见 [02](./02-数据采集层设计.md)、[13](./13-Redis功能与键一览.md)。
