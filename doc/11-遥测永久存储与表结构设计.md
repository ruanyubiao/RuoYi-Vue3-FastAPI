...
| 4 | 20+ 表类型、字段数不同，表怎么设计？ | **不按类型建物理表**；统一帧表 + 数值时序副表，用 `tm_type` / `field_id` 区分 |
...
2. 将 `{device, type, ts, raw_hex, parsed_json, ...}` 投递归档队列，**后台 worker 批量写 MySQL**。
3. 归档失败可重试；热数据仍可用。
...
- **不按 `FF/FC/…` 建多张业务表**：类型用 `tm_type` 字段；新增类型只增配置，不改 DDL。
...
每收到一帧遥测（每种 `tm_type` 一帧）插入一行。

```sql
CREATE TABLE payload_tm_frame (
  id           BIGINT       NOT NULL AUTO_INCREMENT COMMENT '自增ID',
  device_id    VARCHAR(64)  NOT NULL COMMENT '设备ID，如 can:0:0:0',
  tm_type      VARCHAR(8)   NOT NULL COMMENT '遥测表类型 HEX，如 FF、FC',
  ts_ms        BIGINT       NOT NULL COMMENT '帧时间戳(ms)，与 dataId/曲线 score 一致',
  raw_hex      MEDIUMTEXT   NOT NULL COMMENT '完整复合帧 HEX（空格可选，便于审计与重解析）',
  parsed_json  JSON         NOT NULL COMMENT '解析结果 fields 数组',
  field_count  INT          NOT NULL COMMENT '字段个数',
  cfg_version  VARCHAR(64)  DEFAULT NULL COMMENT 'TeleMetryCfg 版本/hash，便于对照重解析',
  source       VARCHAR(16)  DEFAULT 'can' COMMENT '来源：can / inject / ...',
  created_at   DATETIME(3)  DEFAULT CURRENT_TIMESTAMP(3) COMMENT '入库时间',
  PRIMARY KEY (id, ts_ms),
  KEY idx_device_type_ts (device_id, tm_type, ts_ms),
  KEY idx_ts (ts_ms)
...
CREATE TABLE payload_tm_field_num (
  ...
  tm_type    VARCHAR(8)  NOT NULL,
  field_id   VARCHAR(32) NOT NULL COMMENT '如 JGB001',
  ts_ms      BIGINT      NOT NULL,
  value_num  DOUBLE      NOT NULL,
  frame_id   BIGINT      DEFAULT NULL COMMENT '关联 payload_tm_frame.id，可选',
  PRIMARY KEY (device_id, tm_type, field_id, ts_ms),
  KEY idx_type_field_ts (tm_type, field_id, ts_ms),
...
- 20+ 表类型、每表字段数不同：**无需改表**，仅 `tm_type` / `field_id` 不同。
...
| 文件 | 说明 |
| ---- | ---- |
| [`ruoyi-fastapi-backend/sql/ruoyi-fastapi.sql`](../ruoyi-fastapi-backend/sql/ruoyi-fastapi.sql) | MySQL（**优先**） |
...
- 新增 `module_payload` 归档 service / model
- 采集 [`base_collector.py`](../ruoyi-fastapi-backend/module_payload/collectors/base_collector.py) 与注入 [`payload_telemetry_service.py`](../ruoyi-fastapi-backend/module_payload/service/payload_telemetry_service.py) 解析成功后 enqueue
- Worker 批量 `INSERT payload_tm_frame`，异步填充 `payload_tm_field_num`

### Phase 2 — 历史查询 API

- 按 `deviceId + type + [startT, endT]` 查数值点 / 帧
- 曲线页「查询」在时间早于 Redis 窗口时走 MySQL
