"""Redis Key 规范与业务常量（网口无 net: 前缀、总线表键、IO 节流常量）。"""

from __future__ import annotations

from module_payload import constants as c
from module_payload import redis_keys as rk


def test_device_ids() -> None:
    assert rk.can_card_id(3, 0) == 'can:3:0'
    assert rk.can_channel_id(3, 0, 1) == 'can:3:0:1'
    assert rk.serial_id('COM4') == 'serial:COM4'
    assert rk.net_id('udp', '127.0.0.1', 9000) == 'udp:127.0.0.1:9000'
    assert rk.net_id('TCP', '10.0.0.1', 8000) == 'tcp:10.0.0.1:8000'
    assert rk.net_id('', '1.2.3.4', 1) == 'udp:1.2.3.4:1'
    assert not rk.net_id('udp', '1.2.3.4', 9).startswith('net:')
    assert rk.source_id('camera_ctrl') == 'source:camera_ctrl'


def test_queue_and_status_keys() -> None:
    did = 'serial:COM3'
    assert rk.status_key(did) == 'payload:dev:serial:COM3:status'
    assert rk.heartbeat_key(did) == 'payload:dev:serial:COM3:heartbeat'
    assert rk.cmd_queue_key(did) == 'payload:dev:serial:COM3:cmd:queue'
    assert rk.ctrl_queue_key(did) == 'payload:dev:serial:COM3:ctrl'
    assert rk.cmd_result_key(did, 'abc') == 'payload:dev:serial:COM3:cmd:result:abc'
    assert rk.history_key(did) == 'payload:dev:serial:COM3:history'
    assert rk.io_log_key(did) == 'payload:dev:serial:COM3:io:log'
    assert rk.io_log_seq_key(did) == 'payload:dev:serial:COM3:io:seq'
    assert rk.io_stream_key(did) == 'payload:dev:serial:COM3:io:stream:log'
    assert rk.io_stream_seq_key(did) == 'payload:dev:serial:COM3:io:stream:seq'
    assert rk.io_stream_flush_ack_key(did, 'rid') == 'payload:dev:serial:COM3:io:stream:flush:rid'
    assert rk.io_stream_on_key(did) == 'payload:dev:serial:COM3:io:stream:on'
    assert rk.collector_ctrl_id('serial:COM3') == 'serial:COM3'
    assert rk.collector_ctrl_id('udp:1.2.3.4:9') == 'udp:1.2.3.4:9'
    assert rk.collector_ctrl_id('can:3:0:1') == 'can:3:0'


def test_seq_and_tm_keys() -> None:
    assert rk.seq_run_key('rid') == 'payload:seq:run:rid'
    assert rk.seq_run_history_key(7) == 'payload:seq:runs:7'
    assert rk.telemetry_latest_key('d8') == 'payload:tm:D8:latest'
    assert rk.telemetry_latest_ts_key('biu:ff') == 'payload:tm:BIU:FF:latestts'
    assert rk.telemetry_fps_key('d9v17') == 'payload:tm:D9V17:fps'
    assert rk.telemetry_fps_key('d8') == 'payload:tm:D8:fps'
    assert rk.curve_latest_key('FF', 'JGB001') == 'payload:tm:FF:curve'
    assert rk.curve_latest_key('D9V17') == 'payload:tm:D9V17:curve'
    assert rk.archive_queue_key() == 'payload:mysql:archive'
    assert rk.tx_queue_key() == 'payload:mysql:tx'
    assert rk.session_key('serial', 'serial:COM4') == 'payload:session:serial:COM4'
    assert rk.session_key('can', 'can:3:0:0') == 'payload:session:can:3:0:0'
    assert rk.session_key('udp', 'udp:127.0.0.1:9000') == 'payload:session:udp:127.0.0.1:9000'


def test_image_lvds_error_keys() -> None:
    did = 'serial:COM4'
    assert rk.image_key(did) == 'payload:dev:serial:COM4:image:data'
    assert rk.image_meta_key(did) == 'payload:dev:serial:COM4:image:meta'
    assert rk.assembled_latest_key(did) == 'payload:dev:serial:COM4:assembled:latest'
    assert rk.assembled_log_key(did) == 'payload:dev:serial:COM4:assembled:log'
    assert rk.assembled_error_key(did) == 'payload:dev:serial:COM4:assembled:error'
    assert rk.lvds_key(did, 'qd_x') == 'payload:dev:serial:COM4:lvds:qd_x'
    assert rk.error_type_key('tm') == 'payload:error:tm:log'
    assert rk.error_type_latest_key('assembler') == 'payload:error:assembler:latest'
    assert rk.tm_calc_history_key() == 'payload:tm:calc:history'
    assert rk.fileplay_job_key('curve') == 'payload:play:file:curve:job'
    assert rk.fileplay_points_key('abc', 'CAMF008', 'curve') == 'payload:play:file:curve:abc:data'
    assert rk.fileplay_points_key('abc', channel='curve') == rk.fileplay_hash_key('abc', 'curve')
    assert rk.fileplay_hash_key('abc') == 'payload:play:file:history:abc:data'
    assert rk.fileplay_hash_key('abc', 'curve') == 'payload:play:file:curve:abc:data'
    assert not rk.fileplay_hash_key('abc').startswith('payload:tm:')
    assert rk.fileplay_ctrl_key('abc') == 'payload:play:file:history:abc:ctrl'
    assert rk.fileplay_ctrl_key('abc', 'curve') == 'payload:play:file:curve:abc:ctrl'
    assert rk.fileplay_meta_key('abc') == 'payload:play:file:history:abc:meta'
    assert rk.fileplay_worker_status_key('abc', 'curve') == 'payload:play:file:curve:abc:worker'
    assert rk.fileplay_touch_key('abc') == 'payload:play:file:history:abc:touch'
    assert rk.fileplay_hash_from_leaf_key('payload:play:file:history:abc:meta') == 'abc'
    assert rk.fileplay_hash_from_leaf_key('payload:play:file:curve:job') == ''
    assert rk.fileplay_file_prefix('abc') == 'payload:play:file:history:abc:'
    assert rk.fileplay_channel_prefix('history') == 'payload:play:file:history:'
    assert rk.fileplay_channel_prefix('curve') == 'payload:play:file:curve:'
    assert rk.canplay_hash_key('sess1') == 'payload:play:can:sess1'


def test_io_log_constants() -> None:
    assert c.CURVE_MAX_POINTS == 50000
    assert c.CURVE_TS_MAX_AHEAD_MS == 5000
    assert c.IO_LOG_MAX == 1000
    assert c.IO_PREVIEW_COALESCE_S == 1.0
    assert c.STREAM_FLUSH_WAIT_S == 0.4
    assert c.STREAM_FLUSH_ACK_TTL == 5
    assert c.STREAM_IO_FLUSH_BATCH == 64
    # 预览同类 recv 1s 合并；不再按包丢弃（无 IO_LOG_MIN_INTERVAL_S）
    assert not hasattr(c, 'IO_LOG_MIN_INTERVAL_S')


def test_collector_redis_flush_constants() -> None:
    assert c.REDIS_FLUSH_INTERVAL_MS == 2
    assert c.REDIS_FLUSH_COUNT == 20
    assert c.REDIS_FLUSH_FORCE == 100
    assert c.REDIS_TRIM_INTERVAL_S == 1.0
    assert c.REDIS_PIPE_MAX_OPS == 800


def test_bus_tm_keys() -> None:
    assert c.make_bus_tm_key('biu', 'ff') == 'BIU:FF'
    assert c.make_bus_tm_key('XL', 'fd') == 'XL:FD'
    assert c.make_bus_tm_key(None, 'aa') == 'BIU:AA'
    assert c.split_tm_table_key('BIU:FF') == ('biu', 'FF')
    assert c.split_tm_table_key('XL:FD') == ('xl', 'FD')
    assert c.split_tm_table_key('D8') == (None, 'D8')
    assert c.split_tm_table_key('RKDJ') == (None, 'RKDJ')
    assert c.tm_parse_key('BIU:FF') == 'FF'
    assert c.tm_parse_key('ZK') == 'ZK'


def test_infer_src_kind() -> None:
    assert c.infer_src_kind('can:3:0:0') == c.SRC_KIND_CAN
    assert c.infer_src_kind('serial:COM4') == c.SRC_KIND_SERIAL
    assert c.infer_src_kind('COM3') == c.SRC_KIND_SERIAL
    assert c.infer_src_kind('udp:127.0.0.1:9') == c.SRC_KIND_UDP
    assert c.infer_src_kind('tcp:10.0.0.1:8') == c.SRC_KIND_TCP
    assert c.infer_src_kind('http:devtest') == c.SRC_KIND_HTTP
    assert c.infer_src_kind('unknown') == c.SRC_KIND_CAN
    assert c.infer_src_kind('', fallback=c.SRC_KIND_SERIAL) == c.SRC_KIND_SERIAL
    assert c.infer_src_kind('unknown', fallback='') == ''


def test_should_archive_tm_mysql() -> None:
    assert c.should_archive_tm_mysql('can', 'can:3:0:0', c.PARSER_TM_CAN_BIU)
    assert c.should_archive_tm_mysql('http', 'http:devtest', c.PARSER_TM_CAN_BIU)
    assert c.should_archive_tm_mysql('http', 'http:devtest', c.PARSER_TM_CAN_XL)
    assert not c.should_archive_tm_mysql('http', 'http:devtest', c.PARSER_TM_XL_CAMERA)
    assert not c.should_archive_tm_mysql('serial', 'serial:COM3', c.PARSER_TM_XL_CAMERA)
    assert not c.should_archive_tm_mysql('udp', 'udp:127.0.0.1:9', c.PARSER_TM_XL_BOARD)
    assert not c.should_archive_tm_mysql('tcp', 'tcp:10.0.0.1:8', c.PARSER_TM_XL_BOARD)
    assert not c.should_archive_tm_mysql('', 'serial:COM4', c.PARSER_TM_XL_CAMERA)
    assert c.should_archive_tm_mysql('', 'can:3:0:0', c.PARSER_TM_CAN_BIU)
